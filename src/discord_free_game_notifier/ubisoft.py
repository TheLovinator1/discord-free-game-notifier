...
def _get_notified_ubisoft_games() -> list[tuple[DiscordEmbed, str]] | None:
    """Fetch Ubisoft JSON games and build embeds for new active offers.
...
def get_ubisoft_free_games() -> list[tuple[DiscordEmbed, str]] | None:
    ...
    """
    try:
        return _get_notified_ubisoft_games()
    except httpx.TimeoutException as e:
        logger.warning(f"Ubisoft free games JSON timed out, skipping this check: {e}")
        return None
    except httpx.RequestError as e:
        logger.warning(f"Ubisoft free games JSON request failed, skipping this check: {e}")
        return None
